...
def save_checkpoint(state, is_best, filename='ms_checkpoint.pth.tar'):
    """
        根据 is_best 存模型，一般保存 valid acc 最好的模型
    """
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, 'ms_model_best_' + filename)
...
if __name__ == "__main__":
    ...
    print("[Info] DistributedDataParallel~~~~~~~~~~~~~~~~~")
    ...
    for epoch in range(epochs):
        ...
        save_checkpoint({
            ...
            filename='ms_checkpoint_resnet50.pth.tar')
    writer.close()
    end = time.time()
    print("训练结束！！！")
