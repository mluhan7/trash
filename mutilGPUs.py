...
#     if is_best:
#         shutil.copyfile(filename, 'gpus_model_best_' + filename)
...
if __name__ == "__main__":
    # -------------------------------------------- step 1/4 : 加载数据 ---------------------------
    train_dir_list = './model/train.txt'
    # valid_dir_list = './model/val.txt'
    batch_size = 128
    epochs = 50
    num_classes = 214
    ...
    train_loader = DataLoader(dataset=train_data, num_workers=8, pin_memory=True,
                              batch_size=batch_size, sampler=train_sampler)

    # valid_sampler = torch.utils.data.distributed.DistributedSampler(valid_data)
    # valid_loader = DataLoader(dataset=valid_data, num_workers=8, pin_memory=True,
    ...
    for epoch in range(epochs):
        ...
        # }, is_best,
        #     filename='gpus_checkpoint_resnet50.pth.tar')
    end = time.time()
    min = (end - start) / 60
    hour = min / 60
    print("训练结束！！！")
...
# CUDA_VISIBLE_DEVICES=0,1,2,3 python -m torch.distributed.launch --nproc_per_node=4 mutilGPUs.py
# CUDA_VISIBLE_DEVICES=2,3 python -m torch.distributed.launch --nproc_per_node=2 mutilGPUs.py
